...
AmiMotleyTool.prototype = {
      ...
      .then(function(images) {
        ...
        for (var i = 0; i < images.length - 1; i++) {
          ...
          images[i].BlockDeviceMappings.forEach(function(blockDevice) {
            deleteSnapshots.push(self._relentlesslyDeleteSnapshot(blockDevice.Ebs.SnapshotId));
          });
        }
  ...
  /**
   * _relentlesslyDeleteSnapshot (internal method)
   * relentlessly delete snapshot until parent AMI removed.
   * @param {string} snapshotId
   */
  _relentlesslyDeleteSnapshot: function(snapshotId) {
    var self = this;
    return new Promise(function(resolve, reject) {
      var timer = setInterval(function() {
        try {
          self.ec2.deleteSnapshot({
            ...
            if (err.stack.indexOf("InvalidSnapshot.InUse") == -1) {
              // When err does not include "InvalidSnapshot.InUse", abort.
              clearInterval(timer);
              reject(err);
            }
            // When err includes "InvalidSnapshot.InUse", retry.
          });
        } catch (err) {
          clearInterval(timer);
          reject(err);
   ...
   * @param {object} opts : InstanceId and Name required. http://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/EC2.html#createImage-property
   * @param {function} callbackWait()
    ...
    return new Promise(function(resolve, reject) {
      self.ec2.describeImages({
        ...
        image.BlockDeviceMappings.forEach(function(blockDevice) {
          if (!blockDevice.Ebs.SnapshotId) return;
      ...
      var timer = setInterval(function() {
        try {
          ...
          self.ec2.describeImages({
            ...
            return fn(req, timer, resolve, reject);
          }).catch(function(err) {
            clearInterval(timer);
  ...
  waitAvailableAmi: function(imageId, callbackWait) {
    return this._waitAmi(imageId, callbackWait, function(req, timer, resolve, reject) {
  ...
  /**
   * waitUnavailableAmi
   * wait unavailable AMI. *But, It seems to delete AMI immediately after deregisterImage().*
   * @param {string} imageId
   * @param {function} callbackWait()
